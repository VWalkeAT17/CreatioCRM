namespace Terrasoft.Configuration.RightsService
{
	...
	public class RightsHelper
	{
		...
		private void CheckRecordAccessRights(string tableName, string recordId) {
			bool hasSelectEverythingOperation = _userConnection.DBSecurityEngine.GetCanExecuteOperation("CanSelectEverything");
			if (hasSelectEverythingOperation) {
				return; 
			}
			var recordGuid = Guid.Parse(recordId);
			var currentUserRecordRightLevel = _userConnection.DBSecurityEngine.GetEntitySchemaRecordRightLevelsByRecordId(tableName, recordGuid);
			bool hasDirectRecordAccess = currentUserRecordRightLevel.HasFlag(SchemaRecordRightLevels.CanRead) ||
				currentUserRecordRightLevel.HasFlag(SchemaRecordRightLevels.CanEdit) ||
				currentUserRecordRightLevel.HasFlag(SchemaRecordRightLevels.CanDelete);
			if (!hasDirectRecordAccess) {
				throw new SecurityException($"Insufficient rights to access record rights for record {recordId} in schema {tableName}");
			}
		}
		...
		public  List<RecordRight> GetRecordRights(string tableName, string recordId) {
			if (Features.GetIsDisabled("CanReadRecordPermissionsWithoutRights")) {
				CheckRecordAccessRights(tableName, recordId);
			}
