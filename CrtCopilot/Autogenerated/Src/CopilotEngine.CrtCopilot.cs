namespace Creatio.Copilot
{
	...
	internal class CopilotEngine : ICopilotEngine
	{
		...
		private readonly IIntentPromptBuilder _intentPromptBuilder;
		...
		public CopilotEngine(UserConnection userConnection, IGenAICompletionServiceProxy completionService,
				...
				ICopilotSessionResponseDispatcher sessionDispatcher,
				IIntentToolExecutorFactory intentToolExecutorFactory, IIntentPromptBuilder intentPromptBuilder,
				ILlmModelResolver llmModelResolver, IKnwPromptBuilder knwPromptBuilder, 
				ICopilotMessageConfirmationHandler confirmationHandler) {
		...
		private void AdjustSessionSystemIntentPrompt(CopilotSession copilotSession) {
			...
			string systemIntentPrompt = _intentPromptBuilder.GenerateIntentPrompt(SystemIntents, parameters);
		...
		private void SendMessagesToClient(CopilotSession copilotSession) {
			...
			messagesToSend.ForEach(message => message.RootIntentCaption =
				GetIntentCaptionByIntentId(message.RootIntentId));
		...
		private void HandleToolCallsCompletedAndSendSession(List<CopilotMessage> toolMessages, CopilotSession session,
		...
		private async Task HandleApiToolCallsCompletedAsync(List<CopilotMessage> toolMessages, CopilotSession session,
			...
			AdjustSessionSystemIntentPrompt(session);
			session.AddMessages(toolMessages);
			var enableApiToolCalls = Features.GetIsEnabled<GenAIFeatures.EnableApiToolCalls>();
		...
		private CopilotToolContext GetApiSkillToolDefinitions() {
			...
			CopilotToolContext toolContext = _toolProcessor.GetToolContext(systemActions);
			return toolContext;
		}
		...
		private CopilotToolContext GetToolDefinitionsWithAgents(CopilotSession session) {
			...
			CopilotToolContext toolContext = _toolProcessor.GetToolContext(actionMetaItems, intents);
			return toolContext;
		}
		...
		private void HandleConfirmationToolCalls(CopilotMessage userMessage, CopilotSession session) {
			CopilotToolContext toolContext = GetChatSessionToolDefinitions(session);
			List<CopilotMessage> messages = _confirmationHandler.HandleConfirmation(userMessage, session, toolContext);
		...
		private bool TryFindTriggeredIntent(CopilotSession session, List<Guid> rootIntentIds,
				out IToolExecutionTrigger toolExecutionTrigger) {
		...
		public CopilotChatPart SendUserMessage(string content, Guid? copilotSessionId = null,
			...
			try {
				HandleConfirmationToolCalls(userMessage, session);
				SendSession(session, copilotContext);
			} catch (Exception e) {
		...
		public void CompleteAction(Guid copilotSessionId, string actionInstanceUId,
			...
			try {
				HandleToolCallsCompletedAndSendSession(toolMessages, session);
			} catch (Exception e) {
				_log.Error(e);
				throw;
			} finally {
		...
		public async Task<CopilotIntentCallResult> ExecuteIntentAsync(CopilotIntentCall call,
			...
			try {
				...
				JsonSchema jsonSchema = useJsonSchema
					...
					: null;
				var enableApiToolCalls = call.ToolCallOptions.ToolCallsEnabled && Features.GetIsEnabled<GenAIFeatures.EnableApiToolCalls>();
				requestId = await CompleteIntentAsync(session, response, jsonSchema, token, enableApiToolCalls).ConfigureAwait(false);
