namespace Creatio.Copilot
{
	...
	internal class CopilotToolProcessor : ICopilotToolProcessor
	{
		...
		private static ToolDefinition GetToolDefinition(CopilotIntentSchema intentSchema) {
			var intentTypeCaption = intentSchema.Type == CopilotIntentType.Agent ? "agent" : "skill";
			string toolName = $"{intentSchema.Name}_{intentTypeCaption}";
			string description;
		...
		public List<CopilotMessage> HandleToolCalls(ChatCompletionResponse completionResponse, CopilotSession session,
			...
			if (Features.GetIsEnabled<GenAIFeatures.UseActionConfirmations>()) {
				string chatContext = CopilotExtensions.PrepareChatContext(session);
				List<CopilotMessage> toolCallsWithConfirmation =
					_confirmationHandler.GetConfirmationMessages(toolCalls, toolContext, chatContext);
