namespace Creatio.Copilot
{
	...
	internal class KnwPromptBuilder : IKnwPromptBuilder {
		...
		private const int KnwSourceMsgInsertPosition = 3;
		...
		private const string KnwPromptFormat = "You are provided with a list of external Knowledge Sources (RAG).\n" +
			...
			"- You MUST include citations at the end of your response when knowledge sources were used.\n" +
			"- Citations are MANDATORY - always provide them under the heading \"Citations:\".\n" +
			"- Format citations as a bulleted list.\n" +
			"- Each citation MUST strictly follow this format:\n" +
			"  * Display the source name from the source attribute\n" +
			"  * Extract schemaName and Id from location attribute and transform source name to a clickable link to navigate to them" +
			"- If no link can be generated, provide name of the knowledge source." +
			"- Do NOT skip citations - they are required for transparency and verification.\n\n";
		...
		public void AddKnwSourcePrompt(CopilotSession session, List<ChatMessage> messages) {
			...
			if (!string.IsNullOrEmpty(knwPrompt)) {
				int index = messages.Count > KnwSourceMsgInsertPosition ? KnwSourceMsgInsertPosition : messages.Count;
				messages.Insert(index, new ChatMessage(CopilotMessageRole.System, knwPrompt));
			}
		}
